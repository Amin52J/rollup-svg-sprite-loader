...
module.exports = function svgSpriteLoader (options = {}) {
  return {
    ...
    async renderChunk (code, chunk, options) {
      ...
      // eslint-disable-next-line max-len
      const svgNodeInsert = "function loadSpriteFromRollupSVGSpriteLoader() { if (typeof window !== \"undefined\" && window && window.document && window.document.body) { window.document.addEventListener('DOMContentLoaded', function () { const div = document.createElement('div'); div.setAttribute('style', 'position: absolute; height:0; width: 0; overflow: hidden;'); div.innerHTML = JSON.stringify(content); window.document.body.appendChild(div) }); } else { setTimeout(() => { loadSpriteFromRollupSVGSpriteLoader(); }, 50); } } loadSpriteFromRollupSVGSpriteLoader();";
